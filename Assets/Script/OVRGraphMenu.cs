...
///
/// </summary>
public class OVRGraphMenu : MonoBehaviour
{
	...
	public string directions = "Enter a function";
	...
	/*void UpdateFPS()
	{
    	...
    	if( TimeLeft <= 0.0 )
    	{
			...
			if(ShowVRVars == true)// limit gc
				strFPS = System.String.Format("HELLO WORLD: {0:F2}",fps);
    ...
    {
        bool OpenMenu = true;
        if ((OldOpenMenu == false) && (OpenMenu == true))
        {
            if (ShowVRVars == true)
            {
                ShowVRVars = false;
        ...
        // Draw FPS
        GuiHelper.StereoBox(VRVarsSX-70, y += StepY, 300, 50,
                             ref directions, Color.green);

        // Don't draw these vars if CameraController is not present
        if (CameraController != null)
        {
            GuiHelper.StereoBox(VRVarsSX-70, y += StepY, 300, 50,
                             ref equation, Color.white);
            /*GuiHelper.StereoBox(VRVarsSX, y += StepY, VRVarsWidthX, VRVarsWidthY,
